...
function IllustrationPersonalInfo({ handleStateChange, onPersonalInfoChange }) {
  ...
  const handleInputChange = (label, newValue) => {
    ...
    if (['Salutation', 'First Name', 'Middle Name', 'Last Name'].includes(label)) {
      const salutation = label === 'Salutation' ? processedValue : (updatedFormData['Salutation'] || '');
      const firstName = label === 'First Name' ? processedValue : (updatedFormData['First Name'] || '');
      const middleName = label === 'Middle Name' ? processedValue : (updatedFormData['Middle Name'] || '');
      const lastName = label === 'Last Name' ? processedValue : (updatedFormData['Last Name'] || '');
      
      // Combine all name parts, filtering out empty values
      const nameParts = [salutation, firstName, middleName, lastName].filter(part => part && part.trim() !== '');
  ...
  const renderField = (field) => {
        ...
        <>
          ...
          >
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sm={6}>
                    <Autocomplete
                      ...
                      value={statesList.find(state => state.STATE_CODE === addressFormData.state || state.STATE_NAME === addressFormData.state) || null}
            ...
            <DialogActions>
              ...
              <Button onClick={handleAddressModalSave} variant="contained">OK</Button>
            </DialogActions>
          </Dialog>
        </>
      );
    ...
    switch (field.fieldType) {
      case "TextField":
        ...
        if (field.label === 'SSN/Tax ID' || field.label === 'Phone Number' || field.label === 'Email' || field.label === 'Age') {
          let helperText = '';
          let inputProps = {};
          ...
          if (field.label === 'SSN/Tax ID') {
            ...
            helperText = validationErrors[field.label] || 'Format: (XXX) XXX-XXXX';
            inputProps.maxLength = 14;
          } else if (field.label === 'Email') {
            helperText = validationErrors[field.label] || 'Enter a valid email address';
          } else if (field.label === 'Age') {
            helperText = validationErrors[field.label] || 'Numbers only (0-150)';
