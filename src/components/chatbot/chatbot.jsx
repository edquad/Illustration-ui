...
import { getChatbotFormAutofill } from "../../queries/ChatbotQueries";
const Chatbot = ({ onFormAutofill, handleChatbotExpand }) => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      setTimeout(() => {
        ...
        // Simple checks for details
        const hasName = /\b(my name is|i am|this is)\b/i.test(inputValue);
        const hasDOB = /\b\d{1,2}[\/\-\s]\d{1,2}[\/\-\s]\d{2,4}\b/.test(
          inputValue
        );
        ...
        const hasPhone = /\b\d{10}\b/.test(inputValue);
        const hasAddress =
          /\b(st|street|road|rd|apt|avenue|city|state|zip)\b/i.test(inputValue);

        const infoProvided =
          hasName || hasDOB || hasEmail || hasPhone || hasAddress;
  ...
  const handleAutofillForm = async () => {
    try {
      ...
      if (lastBotMsg?.autofillType === "client") {
        ...
        //   "http://localhost:5000/api/form-autofill",
        //   {
        //     method: "POST",
        ...
        const data = await getChatbotFormAutofill(conversation);
        ...
        if (data.status === "success" && onFormAutofill) {
          ...
          setMessages((prev) => [
            ...
            {
              id: prev.length + 1,
              text: "Please review and submit when ready!",
              sender: "bot",
            },
          ]);
        }
